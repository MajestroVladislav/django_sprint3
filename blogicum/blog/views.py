...
from .models import Post as p, Category


def index(request):  # Главная страница
    now = timezone.now()
    posts = p.objects.filter(pub_date__lte=now,
                             is_published=True,
...
def category(request, slug):  # Страница категории
    ...
    now = timezone.now()
    posts = p.objects.filter(category=category, is_published=True,
...
def post_detail(request, pk):  # Страница поста:
    now = timezone.now()
    post = get_object_or_404(p, pk=pk, is_published=True,
